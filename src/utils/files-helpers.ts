...
export class FilesIgnoreHelper {
    ...
    LoadRulesFromFile(path) {
        ...
        if (fs.existsSync(path)) {
            var rules = fs.readFileSync(path).toString().split("\n");
            for (var i in rules) {
                var rule = rules[i];
                if (rule != null && rule != "") {
                    rule = rule.trim();
                    var sharpIndex = rule.indexOf("#");
                    if (sharpIndex == 0) continue;
                    if (sharpIndex > 0) {
    ...
    GetIncludedFiles(directoryInfo) {
        ...
        fs.readdirSync(directoryInfo).forEach(directory => {
            ...
            if (fs.statSync(directory).isDirectory()) {
                ...
                if (this.IsFileIncluded("\\" + relativePath + "\\") && this.IsFileIncluded("\\" + relativePath) && this.IsFileIncluded(locaname + "\\")) {
        ...
        fs.readdirSync(directoryInfo).forEach(file => {
            ...
            if (!fs.statSync(file).isDirectory()) {
                ...
                if (this.IsFileIncluded("\\" + relativePath) && this.IsFileIncluded(locaname)) file_lst.push(relativePath);
            }
        });
