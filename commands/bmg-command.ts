...
export class BmgCommand implements IBmgCommand {
    ...
    constructor(
        ...
        this.Disposable
            = vscode.commands.registerCommand(commandName, () => this.GetNextParam()); 
         context.subscriptions.push(this.Disposable);
    }

    public LoadFunctionOfValue(paramName: string): (IParamsLoader: IParamsLoader) => any {
    ...
    public LoadFunctionOfSelect(paramName: string, options: any[]): (IParamsLoader: IParamsLoader) => any {
        return (paramsLoader: IParamsLoader) => {
            vscode.window.showQuickPick(options,{placeHolder : paramName}).then(v => {
                paramsLoader.command.params[paramName] = v;
                paramsLoader.command.GetNextParam();
            }); 
        }
